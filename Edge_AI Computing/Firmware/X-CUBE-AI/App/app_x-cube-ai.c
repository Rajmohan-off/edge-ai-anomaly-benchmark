...
float sensor_window[TOTAL_SAMPLES];
 // Import the hardware handles from main.c
 ...
 extern uint8_t ina_data[2];

 // Cleaned Data Variables
 extern float accel_g_x, accel_g_y, accel_g_z;
...
int post_process(ai_i8* data[])
{
	float *output = (float *)data[0];
	    ...
	    for(int i = 0; i < 7; i++) {
	        probability[i] = output[i];
	    }

	    prediction=0;

	    // Automatically find the predicted class (Argmax) for all 7 states
	    for (int i = 0; i < 7; i++) {
